...
会输出: SIP 状态 / 微信进程 / 账号目录 / DB 数 / dylib / 内存 scan 是否通 / 拿到几个 key.
SIP 启用 / 微信没登录 / scan 失败时会用中文报错指方向, 比 MCP 启动失败再排查省事.
...
朋友解压后:
1. 终端先跑 `./wxkey doctor` 验证环境 (SIP / 微信 / DB / key 都 OK 再继续).
2. `claude mcp add wx-mcp /path/to/wx-mcp` 注册到 Claude Code.
